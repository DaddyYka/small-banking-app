...
const Withdraw = () => {
  ...
  const handleChange = (event) => {
    ...
    setInputValue(withdrawAmt);
    setWithdraw([...withdraw, { withdrawAmt, timestamp: now.toLocaleString(),type,details }]);

  };

  const handleSubmit=()=> {
    localStorage.setItem("withdraw", JSON.stringify(withdraw),[withdraw])};
